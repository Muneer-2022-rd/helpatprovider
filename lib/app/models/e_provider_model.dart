...
class EProvider extends Model {
  ...
  EProvider.fromJson(Map<String, dynamic>? json) {
    ...
    json!['availability_hours']??[].forEach((element) {
      availabilityHours!.add(AvailabilityHour.fromJson(element));
    });
...
//    addresses = listFromJson(json, 'addresses', (v) => Address.fromJson(v));
    addresses = <Address>[];
    json['addresses']??[].forEach((element) {
...
//    reviews = listFromJson(json, 'e_provider_reviews', (v) => Review.fromJson(v));
    reviews = <Review>[];
    json['e_provider_reviews']??[].forEach((element) {

...
class EServiceFormController extends GetxController {
  final Rx<EService?> eService = EService().obs;
  final RxList<OptionGroup> optionGroups = <OptionGroup>[].obs;
  final RxList<Category> categories = <Category>[].obs;
  final RxList<EProvider> eProviders = <EProvider>[].obs;
  ...
  * */
  ...
  showTimeInputPicker(BuildContext context) async {
    TimeOfDay? newTime;
    newTime = await showTimePicker(
      context: context,
      initialTime: TimeOfDay(hour: hour ?? 7, minute: min ?? 15),
      initialEntryMode: TimePickerEntryMode.input,
    );
    hour = newTime!.hour;
    min = newTime.minute;
    eService.value!.duration = "${newTime.hour}:${newTime.minute}";
    Get.log("${newTime.hour}:${newTime.minute}");
    update();
  }
}
